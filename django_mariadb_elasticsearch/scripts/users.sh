#!/bin/bash
# Vytvoření skupiny a přidáno vlastnictví pro div_app
groupadd div.cz
sudo chown -R :div.cz /div_app/       # Změní skupinu na div.cz
sudo chmod -R g+rw /div_app/         # Přidá skupině práva na čtení a zápis
sudo find /div_app/ -type d -exec chmod g+s {} \;  # Nastaví setgid na všechny složky
echo "cd /div_app" > /etc/profile # Nastaví domovský adresář po připojení na server př. div_app

# Přidání uživatelů a veřejných SSH klíčů z proměnných prostředí, upravit heslo !!!
useradd -m -s /bin/bash -G sudo,div.cz ionno
echo "ionno:lIilililililiIillilililiLIil" | chpasswd 
mkdir -p /home/ionno/.ssh
echo "ssh-rsa  illnezz@centrum.cz" > /home/ionno/.ssh/authorized_keys
chmod 700 /home/ionno/.ssh
chmod 600 /home/ionno/.ssh/authorized_keys
chown -R ionno:ionno /home/ionno
echo 'ionno ALL=(ALL:ALL) NOPASSWD:ALL' >> /etc/sudoers